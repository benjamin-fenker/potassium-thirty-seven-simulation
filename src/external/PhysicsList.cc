...
PhysicsList::PhysicsList() : K37PhysicsList() {
  ...
  decPhysicsList = new K37ParticleConstructor();
}
...
void PhysicsList::AddPhysicsList(const G4String& name) {
  ...
  if (name == "local") {
    ...
    emName = name;
    delete emPhysicsList;
    emPhysicsList = new K37PhysicsConstructor();
...
void PhysicsList::Construct() {
  ConstructParticle();
  ConstructProcess();
  SetCuts();
}

void PhysicsList::SetDefaultCutValue(G4double cut) {
  K37PhysicsList::SetDefaultCutValue(cut);
  SetCutForGamma(cut);
