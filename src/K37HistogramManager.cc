...
:af(0),tree(0),factoryOn(false)
{
  ...
  if(!af)
  {
	  G4cout << " K37HistogramManager::K37HistogramManager() :" << " problem creating the AIDA analysis factory." << G4endl;
  }
#endif 
...
void K37HistogramManager::book()
{
  ...
  ntupl2 = ntf->create( "StripDetectorMinusZ", "StripDetectorMinusZ", "double e");
...
{
  if (nt >= MaxNtupl) {
    G4cout << "---> warning from K37HistogramManager::FillNtuple() : Ntuple " << nt
...
{
  if (nt >= MaxNtupl) {
    G4cout << "---> warning from K37HistogramManager::AddRowNtuple() : Ntuple " << nt
           << " do not exist" << G4endl;
    return;
  }
#ifdef G4ANALYSIS_USE
