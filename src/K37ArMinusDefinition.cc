...
K37Ar37Minus* K37Ar37Minus::Definition()
{
  ...
  //G4ParticleDefinition* anInstance = reinterpret_cast< G4ParticleDefinition*>(pTable->FindParticle(name));
  if (anInstance ==0)
  {
  // create particle
  //
  ...
  //             excitation
	  anInstance = new G4Ions(
		 ...
		 name,        34.4348410923*GeV,          2.5315e-27*GeV,  -1.0*eplus,
		  //name,        34.5*GeV,          2.2*MeV,  -1.0*eplus,
                    0,              +1,             0,
                    0,               0,             0,
   ...
   G4PhaseSpaceDecayChannel* mode = new G4PhaseSpaceDecayChannel("Ar37Minus",1,2,"Ar37Neutral","e-");
