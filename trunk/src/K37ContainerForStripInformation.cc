...
K37ContainerForStripInformation::K37ContainerForStripInformation(K37AllPossibleEventInformation* APEI)
...
void K37ContainerForStripInformation::StoreStripInformation(G4ThreeVector hitPosition, G4double energyDeposited, G4bool trueIfPrimary, G4bool trueIfPlusZDetector)
{
	if(energyDeposited <= 0)
	{
		return;
	}
	//G4cout<<hitPosition<< "    "<< trueIfPrimary<<"  "<<trueIfPlusZDetector<<"  "<<std::fixed<<std::setprecision(10)<<std::setw(12)<< energyDeposited;
	//G4cout<<std::resetiosflags(std::ios_base::fixed);
	//G4cout<<"   "<<this->FindStripNumberForHit(hitPosition.x())<<"   "<< this->FindStripNumberForHit(hitPosition.y())<<G4endl;
	...
	if(trueIfPrimary)
	{
		...
		{
			DetectorPlusZ_X_beta.insert(std::pair<G4int,G4double>(xTemp,energyDeposited));
			DetectorPlusZ_Y_beta.insert(std::pair<G4int,G4double>(yTemp,energyDeposited));
			listPlusZ_X_beta.push_back(xTemp);
			listPlusZ_Y_beta.push_back(yTemp);
		}
		else
		{
			DetectorMinusZ_X_beta.insert(std::pair<G4int,G4double>(xTemp,energyDeposited));
			DetectorMinusZ_Y_beta.insert(std::pair<G4int,G4double>(yTemp,energyDeposited));
	...
	else
	{
		if(trueIfPlusZDetector)
		{
			DetectorPlusZ_X_secondary.insert(std::pair<G4int,G4double>(xTemp,energyDeposited));
			DetectorPlusZ_Y_secondary.insert(std::pair<G4int,G4double>(yTemp,energyDeposited));
			listPlusZ_X_secondary.push_back(xTemp);
			listPlusZ_Y_secondary.push_back(yTemp);
		}
		else
		{
			DetectorMinusZ_X_secondary.insert(std::pair<G4int,G4double>(xTemp,energyDeposited));
			DetectorMinusZ_Y_secondary.insert(std::pair<G4int,G4double>(yTemp,energyDeposited));
...
{
         //for(iteratorOfDetectorMap=DetectorMinusZ_X_beta.begin(); iteratorOfDetectorMap!=DetectorMinusZ_X_beta.end(); iteratorOfDetectorMap++)
	 //{
		 //G4cout<< "-Z beta xStrip Energy[keV]: "<<std::setw(5)<< (*iteratorOfDetectorMap).first<<std::setw(15)<<((*iteratorOfDetectorMap).second)/keV << G4endl;
	 //}
         //for(iteratorOfDetectorMap=DetectorMinusZ_X_secondary.begin(); iteratorOfDetectorMap!=DetectorMinusZ_X_secondary.end(); iteratorOfDetectorMap++)
	 //{
		 //G4cout<< "-Z secondary xStrip Energy[keV]: "<<std::setw(5)<< (*iteratorOfDetectorMap).first<<std::setw(15)<<((*iteratorOfDetectorMap).second)/keV << G4endl;
	 //}
	...
	//{
	G4double energyTemp;
	for (listIterator=listMinusZ_X_beta.begin(); listIterator!=listMinusZ_X_beta.end(); listIterator++)
	{
		energyTemp=0;
		ret = DetectorMinusZ_X_beta.equal_range((*listIterator));
		for (iteratorOfValues=ret.first; iteratorOfValues!=ret.second; ++iteratorOfValues)
		{
	...
	for (listIterator=listMinusZ_Y_beta.begin(); listIterator!=listMinusZ_Y_beta.end(); listIterator++)
	{
		energyTemp=0;
	...
	for (listIterator=listPlusZ_X_beta.begin(); listIterator!=listPlusZ_X_beta.end(); listIterator++)
	{
		energyTemp=0;
	...
	for (listIterator=listPlusZ_Y_beta.begin(); listIterator!=listPlusZ_Y_beta.end(); listIterator++)
	{
		energyTemp=0;
	...
	for (listIterator=listMinusZ_X_secondary.begin(); listIterator!=listMinusZ_X_secondary.end(); listIterator++)
	{
		energyTemp=0;
	...
	for (listIterator=listMinusZ_Y_secondary.begin(); listIterator!=listMinusZ_Y_secondary.end(); listIterator++)
	{
		energyTemp=0;
	...
	for (listIterator=listPlusZ_X_secondary.begin(); listIterator!=listPlusZ_X_secondary.end(); listIterator++)
	{
		energyTemp=0;
	...
	for (listIterator=listPlusZ_Y_secondary.begin(); listIterator!=listPlusZ_Y_secondary.end(); listIterator++)
	{
		energyTemp=0;
...
G4int K37ContainerForStripInformation::FindStripNumberForHit(G4double PositionOfHit)
{
	for (iteratorOfDetectorMap=mapOfDetectorStrips.begin(); iteratorOfDetectorMap != mapOfDetectorStrips.end(); iteratorOfDetectorMap++)
	{
