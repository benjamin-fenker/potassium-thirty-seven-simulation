...
void JTW_Event::MakeEvent()
{
  ...
  while((electron.T < minElectronT) || (fabs(cos(electron.Theta)) < minCosTheta)) {
    while (true)
      {
            ...
            while (true)
              {
                ...
                electron.PmagSquared = (electron.T+electron.Mass)*(electron.T+electron.Mass) - electron.Mass*electron.Mass; // MeV/c with c = 1
