...
:cloudTemperature(_cloudTemperature), cloudSizeStart(_cloudSizeStart), boltzmannConstant(8.61733e-11), massOfAr37(34434.330043344002), speedOfLight(2.99792458e6), cycleTime(4.0)
{
	sigma = sqrt(cloudTemperature*boltzmannConstant*speedOfLight*speedOfLight/massOfAr37);
}
...
void K37CloudSize::makeEvent()
{
	...
	//distanceTraveled = sqrt((endX-startX)*(endX-startX)+(endY-startY)*(endY-startY)+(endZ-startZ)*(endZ-startZ));
}
