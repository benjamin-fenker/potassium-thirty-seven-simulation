...
using std::left; using std::setw; using std::sqrt;
using std::abs;
using std::map;
...
namespace AGG
{
               ...
               const std::string &fileName_)
   {
      ...
      else if(method_ == "root")
      {
         ...
         {
            delete [] eventData; 
            eventData = new Float_t[dataMap.size()];
         }
         else
         {
            eventData = new Float_t[dataMap.size()];
         ...
         {
            delete theNTuple; 
            //theNTuple = new TNtuple("ntuple", "TheTuple",
                  //variableNames.c_str());
            ...
            for (it_data = dataMap.begin(); it_data != dataMap.end(); ++it_data)
            {
               ...
               if(typeBit == "/v")
               {
                  ...
                  //cerr<<"========================================="<<endl;
                  //cerr<<"The Address is: "
                     //<< ((*it_data).second)->GetReferenceV()<<endl;
                  //cerr<<"========================================="<<endl;
               }
            }
   ...
   void Aggregator::ImplementAGG::PrintROOT()
   {
      ...
      //for (it_data = dataMap.begin(); it_data != dataMap.end(); ++it_data)
      //{
         //eventData[counter] = ((*it_data).second)->GetValue();
         //++counter;
      //}
      //theNTuple->Fill(eventData);
      theNTuple->Fill();
   }
