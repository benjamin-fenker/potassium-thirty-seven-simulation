...
G4bool K37StripDetectorSD::ProcessHits(G4Step*aStep, G4TouchableHistory*)
{
	...
	//*******SPENCER'S STUFF******************
        // The second condition here requires that this be the first step in the strip detector.
	if (theTrack->GetParentID() == 0 &&  aStep->GetPreStepPoint()-> GetStepStatus() == fGeomBoundary)
	{
		newHit->SetBound(2.);
