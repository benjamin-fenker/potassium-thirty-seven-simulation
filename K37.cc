...
{
	G4cout<<"================================="<<endl;
	G4cout<<"This is K37 revesion "<<K37_VERSION<<endl;
	G4cout<<"================================="<<endl;
	//G4cout<< "this is a test to see if I can just design a simpler scripting utility"<<G4endl;
	//G4bool shouldwe = 0;
	//G4bool nowewont = 0;
	//G4bool yeswewill = 0;
	//G4cin>> shouldwe>> nowewont>> yeswewill;
	//G4cout<< boolalpha << "should we do it? "<< shouldwe << " now that I think about it "<<nowewont<< " well actually " << yeswewill<<G4endl;
	
	K37AnnihilationPosition* annihilation = new K37AnnihilationPosition("annihilate.txt",1000);
	K37AllPossibleEventInformation* APEI= new K37AllPossibleEventInformation("EventInformation.txt");
	...
	runManager->SetUserAction(gen_action = new K37PrimaryGeneratorAction(detector, APEI, evGen));
	
	K37ListOfVolumeNames *volumesTheBetaEntered = new K37ListOfVolumeNames("volumeNames.txt",1000);

	K37RunAction* run_action;
	runManager->SetUserAction(run_action = new K37RunAction(detector,gen_action,volumesTheBetaEntered,annihilation,APEI,histo));
	
	K37EventAction* event_action;
	runManager->SetUserAction(event_action= new K37EventAction(run_action, volumesTheBetaEntered, APEI, histo)); 
	...
	runManager->SetUserAction(stepping_action = new K37SteppingAction(event_action,volumesTheBetaEntered,annihilation, APEI));

	//get the pointer to the User Interface manager   
	runManager->SetUserAction(new K37StackingAction);	

	system ("rm -rf detectors.root");
  ...
  if (argc!=1)   // batch mode  
    ...
    */
	...
	if(verbosity)	{delete verbosity;}
	if(annihilation){delete annihilation;}
	if(APEI) 	{delete APEI;}
	if(evGen)	{delete evGen;}  
	if(volumesTheBetaEntered)	{delete volumesTheBetaEntered;}
